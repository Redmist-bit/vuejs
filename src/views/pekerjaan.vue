<template>
  <v-container>
    ...
    >
      ...
      <h5>pekerjaan:{{data}}</h5>
  </v-container> 
</template>


<script>
import CariPekerjaan from "../components/CariPekerjaan";
export default {
  components:{
    CariPekerjaan
  },
  ...
  methods: {
    updatePekerjaan(data){
      this.data = data
      // this.pekerjaan.push(data)
      // this.editedItem = data
      // var result=[];
      // const replaceKeys = { NamaPekerjaan: "Perkiraan", HargaJualDefault: "Harga" };
      // const newArray = changeKeyObjects(this.data, replaceKeys);
      const newArray = this.data.map(item => {
        return { Perkiraan: item.NamaPekerjaan, Harga: item.HargaJualDefault };
      });
      ...
      for (let index = 0; index < newArray.length; index++) {
        ...
        // result.push(element)
        // element[0]
        this.pekerjaan.push(element);
      }
      // Object.assign(this.pekerjaan,this.editedItem)
    },
    initialize() {
      this.pekerjaan = [
        {
